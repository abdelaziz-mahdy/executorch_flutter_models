...
This repository contains Python export tools and metadata for ExecuTorch models used by the `executorch_flutter` package. Model `.pte` files and labels are stored as **GitHub Release assets** (not in git) to keep the repo lightweight (~500KB vs 2.5GB previously).
...
- **Git repo**: Only metadata (`index.json`, `versions.json`) and export scripts
- **GitHub Releases**: Model `.pte` files and label files (one release per ExecuTorch version)
- **URL pattern**: `https://github.com/abdelaziz-mahdy/executorch_flutter_models/releases/download/v{version}/{filename}`
- **Labels naming**: `{category}-labels.txt` (e.g., `mobilenet-labels.txt`, `yolo-labels.txt`)

## Project Structure

```
models/
├── versions.json           # Available versions + latest (auto-generated by CI)
├── {version}/index.json    # Model metadata with release download URLs (auto-generated by CI)
