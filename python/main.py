...
def generate_index_json(output_dir="..", version=None):
    ...
    # Base URL for GitHub Releases (models stored as release assets)
    release_base_url = f"https://github.com/abdelaziz-mahdy/executorch_flutter_models/releases/download/v{version}" if version else "https://github.com/abdelaziz-mahdy/executorch_flutter_models/releases/download/latest"

    # Base URL for raw files still in the repo (index.json, versions.json)
    repo_base_url = "https://raw.githubusercontent.com/abdelaziz-mahdy/executorch_flutter_models/main"
    if version:
        repo_base_url = f"{repo_base_url}/{version}"

    # Model categories and their configurations
    # Labels are stored as release assets with category prefix: mobilenet-labels.txt, yolo-labels.txt
    model_configs = {
        "mobilenet": {
            "inputSize": 224,
            "labelsFile": "labels.txt",
            "labelsUrl": f"{release_base_url}/mobilenet-labels.txt",
        },
        "yolo": {
            "inputSize": 640,
            "labelsFile": "labels.txt",
    ...
    for category, config in model_configs.items():
        ...
        for pte_file in sorted(category_dir.glob("*.pte")):
            ...
            model_entry = {
                ...
                "remoteUrl": f"{release_base_url}/{file_name}",
            }
        ...
        if config["labelsFile"]:
            ...
            if labels_path.exists():
                ...
                labels.append({
                    ...
                    "remoteUrl": f"{release_base_url}/{category}-{config['labelsFile']}",
                })
